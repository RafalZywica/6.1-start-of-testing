...
public class CrudAppTestSuite {
    ...
    private boolean checkTaskExistsInTrello(String taskName) throws InterruptedException {
        ...
        driverTrello.findElement(By.id("user")).sendKeys("jaren80@wp.pl");		        // [3]
        driverTrello.findElement(By.id("password")).sendKeys("Trello123");		    // [4]
        WebElement el = driverTrello.findElement(By.id("login"));
        el.submit();									                                // [5]

        Thread.sleep(4000);								                            // [6]

        driverTrello.findElement(By.id("password")).sendKeys("Trello123");		    // [7]
        driverTrello.findElement(By.id("login-submit")).submit();

        Thread.sleep(4000);								                            // [8]

        driverTrello.findElements(By.xpath("//a[@class=\"board-title\"]")).stream()  // [9]
                .filter(aHref -> aHref.findElements(By.xpath(".//div[@title=\"Pierwsza tablica\"]")).size() > 0)
